        ...
        return (
          <button
            ...
            className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors cursor-pointer ${
              active ? 'bg-[#009eda]/15 text-[#009eda]' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            {label}
          </button>
...
function MiniSelect({ options, value, onChange, placeholder, multiple = false }) {
  return (
    <select
      ...
      multiple={multiple}
      className="px-2.5 py-1.5 rounded-lg border border-black/[0.12] bg-white text-[11px] text-gray-700 outline-none focus:border-[#009eda] cursor-pointer"
    ...
    <div className="sticky top-0 z-10 bg-white py-3 border-b border-black/[0.06]">
      ...
      {activeCount > 0 && (
        <div className="flex items-center gap-2 mt-2">
          ...
          <button
            onClick={clearAll}
            className="flex items-center gap-0.5 text-[10px] text-[#009eda] hover:text-[#0089c4] transition-colors cursor-pointer"
          >
