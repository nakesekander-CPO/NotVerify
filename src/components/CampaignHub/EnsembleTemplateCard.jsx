  ...
  return (
    <button
      ...
      className={`w-full text-left p-3.5 rounded-xl border transition-all cursor-pointer ${
        selected
          ? 'border-[#3D16FA] bg-[#3D16FA]/[0.04] ring-1 ring-[#3D16FA]/30'
          : 'border-black/[0.10] bg-white hover:border-[#3D16FA]/40 hover:bg-[#3D16FA]/[0.02]'
      }`}
    >
      <div className="flex items-start gap-3">
        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${selected ? 'bg-[#3D16FA]/10 text-[#3D16FA]' : 'bg-gray-100 text-gray-500'}`}>
          <Icon size={15} />
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <p className={`text-[13px] font-semibold leading-tight ${selected ? 'text-gray-900' : 'text-gray-800'}`}>{template.name}</p>
            {selected && <CheckCircle2 size={13} className="text-[#3D16FA] shrink-0" />}
          </div>
