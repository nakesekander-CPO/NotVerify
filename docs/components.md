...
| `WorkflowStepper` | `WorkflowStepper.jsx` | 132 | Sticky secondary nav showing the 6-step project pipeline (Intelligence Hub → Agent Selection → Processing → Quality Check → Review → Cortex). Completed steps show a green check; active step pulses. Shows a one-time tooltip hint stored in `localStorage`. | `currentState` (`'dashboard'` \| `'reading'` \| `'processing'` \| `'narrative'` \| `'human-review'` \| `'org-brain'`) |
